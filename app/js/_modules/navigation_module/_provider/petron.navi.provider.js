          ...
          return {
            ...
            lookUpAddress: function(address, type) {
              ...
              if (type === 'country') {
                ...
                params =
                  '&osm_tag=:city&osm_tag=:village&osm_tag=:town';
              } else if (type === 'street') {
                params =
                  '&osm_tag=building&osm_tag=residential&osm_tag=tertiary&' +
                  'osm_tagplace&osm_tag=tourism&osm_tag=amenity&osm_tag=emergency&' +
                  'osm_tag=cuisine';
              }

              address = encodeURI(address);
              var promise = $http.get(sprintf(_geocoder, address) +
                params, {
            ...
            getRoutes: function(route) {
              route = route.start.lat + ',' + route.start.lon + ';' +
                route.dest
                .lat + ',' + route.dest.lon;
              return $http.get(sprintf(_routingmachine, route)).then(
                function(
                  data) {
                  if (data.data && data.status === 200) {
                    return data.data;
