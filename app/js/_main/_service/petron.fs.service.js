							...
							async.waterfall([
								function(callback) {
									...
									mm(readableStream, function(err, tags) {
										...
										if (tags.picture.length > 0 && tags.picture[0].data) {
											img = tags.picture[0].data.toString('base64');
								...
								function(tags, callback) {
									...
									if (file.split('.').pop().match(/mp3|aac|wma|m4a/)) {
										tags.path = $sce.trustAsResourceUrl(encodeURI(path.join(fpath,
											file)));
								...
								function(callback) {
									...
									process.then(function(tags) {
										...
										ffprobe(fpath + '/' + file, {
											...
											if (tags.picture && tags.picture.length > 0 && tags.picture[
													0].data) {
												img = tags.picture[0].data.toString('base64');
												type = tags.picture[0].format;
											}
